export default function InstallmentsPage() {
  return (
    <div className="space-y-8">
      <section className="overflow-hidden rounded-[28px] border border-[#d9d2c8] bg-gradient-to-r from-[#e8eef7] via-[#d7e2f3] to-[#c2d3ea]">
      ...
      <section className="rounded-[24px] border border-[#d9d2c8] bg-white p-6 md:p-8">
        ...
        <ul className="mt-5 space-y-2 text-sm leading-relaxed text-zinc-800 md:text-base">
          <li>
            <span className="font-semibold text-[#2e5fa8]">Рассрочка 0%:</span> на 4, 6 или 10 месяцев.
          </li>
          <li>
            <span className="font-semibold text-[#2e5fa8]">Классический кредит:</span> до 500 000 ₽.
          </li>
          <li>
            <span className="font-semibold text-[#2e5fa8]">Оформление:</span> за несколько минут, без скрытых комиссий.
          </li>
        </ul>
        <div className="mt-6 grid gap-3 sm:grid-cols-2 md:grid-cols-4">
          {["Т-Банк", "Альфа-Банк", "Сбер", "ВТБ"].map((bank) => (
            <div
              key={bank}
              className="rounded-xl border border-[#cfd8e6] bg-[#f3f7fc] px-4 py-3 text-center text-sm font-semibold text-[#1d3f6e]"
            >
              {bank}
            </div>
          ))}
